...
// Получаем ID истребителя из GET параметра
$fighter_id = isset($_GET['id']) ? intval($_GET['id']) : 0;

// Определяем страницу для кнопки "Назад" на основе параметра gen
$back_url = '../vehicle/istrebiteli.php?gen=1'; // значение по умолчанию

if (isset($_GET['gen'])) {
    $gen = intval($_GET['gen']);
    $back_url = '../vehicle/istrebiteli.php?gen=' . $gen;
} elseif (isset($_GET['from']) && $_GET['from'] == 'istrebiteli') {
    // Если пришли из общего списка истребителей
    $back_url = '../vehicle/istrebiteli.php?gen=1';
}
...
// Получаем полную информацию об истребителе ДО всех обработок
$query = "
    SELECT 
        v.ID, 
        v.Name, 
...
if ($fighter) {
    $fighter['Name'] = clean_output($fighter['Name']);
    $fighter['History'] = clean_output($fighter['History'] ?? '');
}
...
// Обработка обновления данных истребителя
if ($can_edit_article && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_fighter'])) {
    // Получаем текущие данные истребителя ДО обновления для логирования
    $old_data_stmt = $mysqli->prepare("
        ...
        WHERE v.ID = ?
    ");
    $old_data_stmt->bind_param("i", $fighter_id);
    $old_data_stmt->execute();
    $old_data_result = $old_data_stmt->get_result();
    $old_fighter_data = $old_data_result->fetch_assoc();
    $old_data_stmt->close();
    ...
    try {
        ...
        $old_values_for_log = [
            'Name' => $old_fighter_data['Name'] ?? '',
            'History' => $old_fighter_data['History'] ?? '',
            'Country_ID' => $old_fighter_data['Country_ID'] ?? 0,
            'Weapon_ID' => $old_fighter_data['Weapon_ID'] ?? null,
            'War_ID' => $old_fighter_data['War_ID'] ?? null,
            'Year_of_commissioning' => $old_fighter_data['Year_of_commissioning'] ?? 0,
            'Year_of_decommissioning' => $old_fighter_data['Year_of_decommissioning'] ?? 0,
            'In_service' => $old_fighter_data['In_service'] ?? 0
        ];
        ...
        if ($old_fighter_data) {
            $old_values_for_log['CountryName'] = $old_fighter_data['CountryName'] ?? '';
            $old_values_for_log['WeaponName'] = $old_fighter_data['WeaponName'] ?? '';
            $old_values_for_log['WeaponType'] = $old_fighter_data['WeaponType'] ?? '';
            $old_values_for_log['WeaponCalibre'] = $old_fighter_data['WeaponCalibre'] ?? '';
            $old_values_for_log['WarPeriod'] = $old_fighter_data['WarPeriod'] ?? '';
        }
        ...
        $description = "Истребитель '{$old_fighter_data['Name']}' отредактирован пользователем ID: {$_SESSION['user_id']}";
        
        // Записываем лог
        $logger->logUpdate('Vehicle', $fighter_id, $old_values_for_log, $new_values_for_log, $description);
        
        $_SESSION['favorite_message'] = 'Данные истребителя успешно обновлены!';
    ...
    $redirect_params = 'id=' . $fighter_id;
    if (isset($_GET['gen'])) {
        $redirect_params .= '&gen=' . $_GET['gen'];
    }
    if (isset($_GET['from'])) {
        $redirect_params .= '&from=' . $_GET['from'];
    }
    
    header("Location: fighter_detail.php?" . $redirect_params);
    exit();
}
...
if (isset($_SESSION['user_id']) && isset($_POST['toggle_favorite'])) {
    ...
    $check_favorite->bind_param("ii", $user_id, $fighter_id);
    ...
    if ($check_favorite->num_rows > 0) {
        ...
        $delete_favorite->bind_param("ii", $user_id, $fighter_id);
        $delete_favorite->execute();
        $is_favorite = false;
        
        // ЛОГИРОВАНИЕ: Удаление из избранного
        $logger->log('favorite_remove', 'user_favorites', $fighter_id, 
            "Истребитель '{$fighter['Name']}' удален из избранного пользователем ID: $user_id");
        ...
        $add_favorite->bind_param("ii", $user_id, $fighter_id);
        $add_favorite->execute();
        $is_favorite = true;
        
        // ЛОГИРОВАНИЕ: Добавление в избранное
        $logger->log('favorite_add', 'user_favorites', $fighter_id, 
            "Истребитель '{$fighter['Name']}' добавлен в избранное пользователем ID: $user_id");
...
if ($can_edit_article && isset($_POST['upload_image'])) {
    if (isset($_FILES['new_image']) && $_FILES['new_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($file_extension, $allowed_extensions)) {
            ...
            $new_filename = 'fighter_' . $fighter_id . '_' . time() . '.' . $file_extension;
            ...
            if ($_FILES['new_image']['size'] <= 5 * 1024 * 1024) {
                if (move_uploaded_file($_FILES['new_image']['tmp_name'], $upload_path)) {
                    ...
                    $check_image->bind_param("i", $fighter_id);
                    $check_image->execute();
                    ...
                    if ($check_image->num_rows > 0) {
                        ...
                        $update_image->bind_param("ssi", $relative_path, $image_type, $fighter_id);
                        
                        if ($update_image->execute()) {
                            // ЛОГИРОВАНИЕ: Записываем обновление изображения
                            $description = "Обновлено изображение для истребителя '{$fighter['Name']}'";
                            $logger->logUpdate('Vehicle', $fighter_id, 
                        ...
                        $insert_image->bind_param("iss", $fighter_id, $relative_path, $image_type);
                        
                        if ($insert_image->execute()) {
                            // ЛОГИРОВАНИЕ: Записываем загрузку изображения
                            $description = "Загружено новое изображение для истребителя '{$fighter['Name']}'";
                            $logger->logUpdate('Vehicle', $fighter_id, 
    ...
    header("Location: fighter_detail.php?id=" . $fighter_id . (isset($_GET['gen']) ? '&gen=' . $_GET['gen'] : '') . (isset($_GET['from']) ? '&from=' . $_GET['from'] : ''));
    exit();
}
...
if ($can_edit_article && isset($_POST['delete_image'])) {
    ...
    if ($check_image->num_rows > 0) {
        ...
        $description = "Удалено изображение для истребителя '{$fighter['Name']}'";
        $logger->logUpdate('Vehicle', $fighter_id, 
        ...
        if ($file_deleted) {
            ...
            $delete_image->bind_param("i", $fighter_id);
...
if ($fighter['War_ID']) {
    ...
    $war_stmt->bind_param("i", $fighter['War_ID']);
    $war_stmt->execute();
...
if (isset($_SESSION['user_id'])) {
    $check_favorite = $mysqli->prepare("SELECT id FROM user_favorites WHERE user_id = ? AND article_id = ?");
    $check_favorite->bind_param("ii", $_SESSION['user_id'], $fighter_id);
    $check_favorite->execute();
    $check_favorite->store_result();
...
if ($image_stmt) {
    $image_stmt->bind_param("i", $fighter_id);
    $image_stmt->execute();
...
<head>
    ...
    <title><?php echo htmlspecialchars($fighter['Name'], ENT_QUOTES, 'UTF-8'); ?> - Военный справочник</title>
        ...
        <div class="vehicle-detail">
            <div class="vehicle-header">
                <div class="image-section">
                    <div class="vehicle-image-large" id="vehicleImageContainer">
                        <?php if ($image_exists): ?>
                            <img src="../<?php echo htmlspecialchars($image_path); ?>" alt="<?php echo htmlspecialchars($fighter['Name'], ENT_QUOTES, 'UTF-8'); ?>" id="vehicleImage">
                        <?php else: ?>
                ...
                <div class="vehicle-info">
                    <div class="vehicle-title-section">
                        <h1 class="vehicle-name"><?php echo htmlspecialchars($fighter['Name'], ENT_QUOTES, 'UTF-8'); ?></h1>
                        <?php if ($can_edit_article): ?>
                    ...
                    <div class="vehicle-specs-grid">
                        <?php if (!empty($fighter['CountryName'])): ?>
                            <div class="spec-card">
                                ...
                                <div class="spec-value"><?php echo htmlspecialchars($fighter['CountryName']); ?></div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($fighter['ClassName'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Класс</div>
                                <div class="spec-value"><?php echo htmlspecialchars($fighter['ClassName']); ?></div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($fighter['WeaponName']) || !empty($fighter['WeaponType']) || !empty($fighter['WeaponCalibre'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Вооружение</div>
                                <div class="spec-value">
                                    <?php if (!empty($fighter['WeaponName'])): ?>
                                        <?php echo htmlspecialchars($fighter['WeaponName'], ENT_QUOTES, 'UTF-8'); ?>
                                    <?php endif; ?>
                                </div>
                                <div class="weapon-details">
                                    <?php if (!empty($fighter['WeaponType'])): ?>
                                        <div class="weapon-detail">Тип: <?php echo htmlspecialchars($fighter['WeaponType'], ENT_QUOTES, 'UTF-8'); ?></div>
                                    <?php endif; ?>
                                    <?php if (!empty($fighter['WeaponCalibre'])): ?>
                                        <div class="weapon-detail">Калибр: <?php echo htmlspecialchars($fighter['WeaponCalibre'], ENT_QUOTES, 'UTF-8'); ?></div>
                                    <?php endif; ?>
                                </div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($fighter['WarPeriod'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Участия в войнах или конфликтах</div>
                                <div class="spec-value"><?php echo htmlspecialchars($fighter['WarPeriod'], ENT_QUOTES, 'UTF-8'); ?></div>
                            </div>
                        <?php endif; ?>
                        ...
                        <?php if (!empty($fighter['Year_of_commissioning'])): ?>
                            <div class="spec-card">
                                ...
                                <div class="spec-value"><?php echo htmlspecialchars($fighter['Year_of_commissioning']); ?></div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($fighter['Year_of_decommissioning'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Конец службы</div>
                                <div class="spec-value"><?php echo htmlspecialchars($fighter['Year_of_decommissioning']); ?></div>
                            </div>
                        <?php endif; ?>
            ...
            <?php if ($can_edit_article): ?>
                <div class="edit-form-container" id="editFormContainer" style="display: none;">
                    <form method="POST" class="edit-form" id="editForm">
                        <h2 class="edit-form-title">Редактирование данных истребителя</h2>
                        
                        <div class="form-grid">
                            <div class="form-group">
                                <label class="form-label required">Название истребителя</label>
                                <input type="text" name="name" value="<?php echo htmlspecialchars($fighter['Name'], ENT_QUOTES, 'UTF-8'); ?>" class="form-input" required>
                            </div>
                                ...
                                <select name="country_id" class="form-select" required>
                                    ...
                                    <?php foreach ($countries as $country): ?>
                                        <option value="<?php echo $country['ID']; ?>" 
                                            <?php echo $fighter['Country_ID'] == $country['ID'] ? 'selected' : ''; ?>>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="year_commissioning" value="<?php echo !empty($fighter['Year_of_commissioning']) ? htmlspecialchars($fighter['Year_of_commissioning']) : ''; ?>" class="form-input" min="1900" max="2030">
                            </div>
                            
                            <div class="form-group">
                                <label class="form-label">Год снятия с эксплуатации</label>
                                <input type="number" name="year_decommissioning" value="<?php echo !empty($fighter['Year_of_decommissioning']) ? htmlspecialchars($fighter['Year_of_decommissioning']) : ''; ?>" class="form-input" min="1900" max="2030">
                                <div class="form-hint">Оставьте пустым, если истребитель всё ещё в эксплуатации</div>
                            </div>
                        </div>
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-grid">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="weapon_calibre" value="<?php echo !empty($current_weapon['Calibre']) ? htmlspecialchars($current_weapon['Calibre'], ENT_QUOTES, 'UTF-8') : ''; ?>" class="form-input" placeholder="Например: 30 мм, 20 мм">
                                </div>
                                
                                <div class="form-group">
                                    <label class="form-label">Название вооружения</label>
                                    <input type="text" name="weapon_name" value="<?php echo !empty($current_weapon['Name']) ? htmlspecialchars($current_weapon['Name'], ENT_QUOTES, 'UTF-8') : ''; ?>" class="form-input" placeholder="Например: ГШ-30-1, Р-73">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="war_name" value="<?php echo !empty($current_war['Name']) ? htmlspecialchars($current_war['Name'], ENT_QUOTES, 'UTF-8') : ''; ?>" class="form-input" placeholder="Например: Вторая мировая война, Корейская война">
                            </div>
                        </div>
                        
                        <div class="form-group full-width">
                            <label class="form-label">История и характеристики</label>
                            <textarea name="history" class="form-textarea" rows="8"><?php echo htmlspecialchars($fighter['History'] ?? ''); ?></textarea>
                        </div>
                        
                        <div class="form-actions">
                            <button type="submit" name="update_fighter" class="form-btn save-btn">💾 Сохранить изменения</button>
            ...
            <div class="history-section">
                ...
                <div class="history-content">
                    <?php echo nl2br(htmlspecialchars($fighter['History'] ?? 'Описание отсутствует')); ?>
                </div>
            </div>

            <div class="action-buttons">
                <a href="<?php echo $back_url; ?>" class="back-button">← Назад к списку истребителей</a>
            ...
            if (editForm) {
                editForm.addEventListener('submit', function(e) {
                    ...
                    if (!nameInput.value.trim()) {
                        e.preventDefault();
                        alert('Пожалуйста, введите название истребителя');
                        nameInput.focus();
                        return;
                    }
