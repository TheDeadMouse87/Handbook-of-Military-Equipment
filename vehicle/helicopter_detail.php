...
// Получаем ID вертолета из GET параметра
$helicopter_id = isset($_GET['id']) ? intval($_GET['id']) : 0;

// Определяем страницу для кнопки "Назад" на основе параметра type
$back_url = '../vehicle/helicopters.php?type=6'; // значение по умолчанию (Боевые вертолёты)

if (isset($_GET['type'])) {
    $type = intval($_GET['type']);
    $back_url = '../vehicle/helicopters.php?type=' . $type;
} elseif (isset($_GET['from']) && $_GET['from'] == 'helicopters') {
    // Если пришли из общего списка вертолетов
    $back_url = '../vehicle/helicopters.php?type=6';
}
...
// Обработка обновления данных вертолета
if ($can_edit_article && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_helicopter'])) {
    // Получаем текущие данные вертолета ДО обновления для логирования
    $old_data_stmt = $mysqli->prepare("
        ...
        WHERE v.ID = ?
    ");
    $old_data_stmt->bind_param("i", $helicopter_id);
    $old_data_stmt->execute();
    $old_data_result = $old_data_stmt->get_result();
    $old_helicopter_data = $old_data_result->fetch_assoc();
    $old_data_stmt->close();
    ...
    try {
        ...
        $old_values_for_log = [
            'Name' => $old_helicopter_data['Name'] ?? '',
            'History' => $old_helicopter_data['History'] ?? '',
            'Country_ID' => $old_helicopter_data['Country_ID'] ?? 0,
            'Weapon_ID' => $old_helicopter_data['Weapon_ID'] ?? null,
            'War_ID' => $old_helicopter_data['War_ID'] ?? null,
            'Year_of_commissioning' => $old_helicopter_data['Year_of_commissioning'] ?? 0,
            'Year_of_decommissioning' => $old_helicopter_data['Year_of_decommissioning'] ?? 0,
            'In_service' => $old_helicopter_data['In_service'] ?? 0
        ];
        ...
        if ($old_helicopter_data) {
            $old_values_for_log['CountryName'] = $old_helicopter_data['CountryName'] ?? '';
            $old_values_for_log['WeaponName'] = $old_helicopter_data['WeaponName'] ?? '';
            $old_values_for_log['WeaponType'] = $old_helicopter_data['WeaponType'] ?? '';
            $old_values_for_log['WeaponCalibre'] = $old_helicopter_data['WeaponCalibre'] ?? '';
            $old_values_for_log['WarPeriod'] = $old_helicopter_data['WarPeriod'] ?? '';
        }
        ...
        $description = "Вертолет '{$old_helicopter_data['Name']}' отредактирован пользователем ID: {$_SESSION['user_id']}";
        
        // Записываем лог
        $logger->logUpdate('Vehicle', $helicopter_id, $old_values_for_log, $new_values_for_log, $description);
        
        $_SESSION['favorite_message'] = 'Данные вертолета успешно обновлены!';
    ...
    $redirect_params = 'id=' . $helicopter_id;
    if (isset($_GET['type'])) {
    ...
    header("Location: helicopter_detail.php?" . $redirect_params);
    exit();
}
...
if (isset($_SESSION['user_id']) && isset($_POST['toggle_favorite'])) {
    ...
    $check_favorite->bind_param("ii", $user_id, $helicopter_id);
    ...
    if ($check_favorite->num_rows > 0) {
        ...
        $delete_favorite->bind_param("ii", $user_id, $helicopter_id);
        $delete_favorite->execute();
        $is_favorite = false;
        
        // ЛОГИРОВАНИЕ: Удаление из избранного
        $logger->log('favorite_remove', 'user_favorites', $helicopter_id, 
            "Вертолет '{$helicopter['Name']}' удален из избранного пользователем ID: $user_id");
        ...
        $add_favorite->bind_param("ii", $user_id, $helicopter_id);
        $add_favorite->execute();
        $is_favorite = true;
        
        // ЛОГИРОВАНИЕ: Добавление в избранное
        $logger->log('favorite_add', 'user_favorites', $helicopter_id, 
            "Вертолет '{$helicopter['Name']}' добавлен в избранное пользователем ID: $user_id");
...
if ($can_edit_article && isset($_POST['upload_image'])) {
    if (isset($_FILES['new_image']) && $_FILES['new_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($file_extension, $allowed_extensions)) {
            ...
            $new_filename = 'helicopter_' . $helicopter_id . '_' . time() . '.' . $file_extension;
            ...
            if ($_FILES['new_image']['size'] <= 5 * 1024 * 1024) {
                if (move_uploaded_file($_FILES['new_image']['tmp_name'], $upload_path)) {
                    ...
                    $check_image->bind_param("i", $helicopter_id);
                    $check_image->execute();
                    ...
                    if ($check_image->num_rows > 0) {
                        ...
                        $update_image->bind_param("ssi", $relative_path, $image_type, $helicopter_id);
                        
                        if ($update_image->execute()) {
                            // ЛОГИРОВАНИЕ: Записываем обновление изображения
                            $description = "Обновлено изображение для вертолета '{$helicopter['Name']}'";
                            $logger->logUpdate('Vehicle', $helicopter_id, 
                        ...
                        $insert_image->bind_param("iss", $helicopter_id, $relative_path, $image_type);
                        
                        if ($insert_image->execute()) {
                            // ЛОГИРОВАНИЕ: Записываем загрузку изображения
                            $description = "Загружено новое изображение для вертолета '{$helicopter['Name']}'";
                            $logger->logUpdate('Vehicle', $helicopter_id, 
    ...
    header("Location: helicopter_detail.php?id=" . $helicopter_id . (isset($_GET['type']) ? '&type=' . $_GET['type'] : '') . (isset($_GET['from']) ? '&from=' . $_GET['from'] : ''));
    exit();
}
...
if ($can_edit_article && isset($_POST['delete_image'])) {
    ...
    if ($check_image->num_rows > 0) {
        ...
        $description = "Удалено изображение для вертолета '{$helicopter['Name']}'";
        $logger->logUpdate('Vehicle', $helicopter_id, 
        ...
        if ($file_deleted) {
            ...
            $delete_image->bind_param("i", $helicopter_id);
...
// Получаем полную информацию о вертолете
$query = "
    SELECT 
        v.ID, 
        v.Name, 
    ...
    LEFT JOIN War war ON v.War_ID = war.ID
    LEFT JOIN Service s ON v.Service_ID = s.ID
    WHERE v.ID = ? AND v.Class_ID = 7
";

$stmt = $mysqli->prepare($query);
$stmt->bind_param("i", $helicopter_id);
$stmt->execute();
$result = $stmt->get_result();
$helicopter = $result->fetch_assoc();
...
if ($helicopter) {
    $helicopter['Name'] = clean_output($helicopter['Name']);
    $helicopter['History'] = clean_output($helicopter['History'] ?? '');
}
...
$current_war = null;
if ($helicopter['War_ID']) {
    $war_stmt = $mysqli->prepare("SELECT Name FROM War WHERE ID = ?");
    $war_stmt->bind_param("i", $helicopter['War_ID']);
    $war_stmt->execute();
...
if (isset($_SESSION['user_id'])) {
    $check_favorite = $mysqli->prepare("SELECT id FROM user_favorites WHERE user_id = ? AND article_id = ?");
    $check_favorite->bind_param("ii", $_SESSION['user_id'], $helicopter_id);
    $check_favorite->execute();
    $check_favorite->store_result();
...
if ($image_stmt) {
    $image_stmt->bind_param("i", $helicopter_id);
    $image_stmt->execute();
...
<head>
    ...
    <title><?php echo htmlspecialchars($helicopter['Name'], ENT_QUOTES, 'UTF-8'); ?> - Военный справочник</title>
        ...
        <div class="vehicle-detail">
            <div class="vehicle-header">
                <div class="image-section">
                    <div class="vehicle-image-large" id="vehicleImageContainer">
                        <?php if ($image_exists): ?>
                            <img src="../<?php echo htmlspecialchars($image_path); ?>" alt="<?php echo htmlspecialchars($helicopter['Name'], ENT_QUOTES, 'UTF-8'); ?>" id="vehicleImage">
                        <?php else: ?>
                ...
                <div class="vehicle-info">
                    <div class="vehicle-title-section">
                        <h1 class="vehicle-name"><?php echo htmlspecialchars($helicopter['Name'], ENT_QUOTES, 'UTF-8'); ?></h1>
                        <?php if ($can_edit_article): ?>
                    ...
                    <div class="vehicle-specs-grid">
                        <?php if (!empty($helicopter['CountryName'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Страна</div>
                                <div class="spec-value"><?php echo htmlspecialchars($helicopter['CountryName']); ?></div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($helicopter['ClassName'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Класс</div>
                                <div class="spec-value"><?php echo htmlspecialchars($helicopter['ClassName']); ?></div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($helicopter['WeaponName']) || !empty($helicopter['WeaponType']) || !empty($helicopter['WeaponCalibre'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Вооружение</div>
                                <div class="spec-value">
                                    <?php if (!empty($helicopter['WeaponName'])): ?>
                                        <?php echo htmlspecialchars($helicopter['WeaponName'], ENT_QUOTES, 'UTF-8'); ?>
                                    <?php endif; ?>
                                </div>
                                <div class="weapon-details">
                                    <?php if (!empty($helicopter['WeaponType'])): ?>
                                        <div class="weapon-detail">Тип: <?php echo htmlspecialchars($helicopter['WeaponType'], ENT_QUOTES, 'UTF-8'); ?></div>
                                    <?php endif; ?>
                                    <?php if (!empty($helicopter['WeaponCalibre'])): ?>
                                        <div class="weapon-detail">Калибр: <?php echo htmlspecialchars($helicopter['WeaponCalibre'], ENT_QUOTES, 'UTF-8'); ?></div>
                                    <?php endif; ?>
                                </div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($helicopter['WarPeriod'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Участия в войнах или конфликтах</div>
                                <div class="spec-value"><?php echo htmlspecialchars($helicopter['WarPeriod'], ENT_QUOTES, 'UTF-8'); ?></div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($helicopter['Year_of_commissioning'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Начало службы</div>
                                <div class="spec-value"><?php echo htmlspecialchars($helicopter['Year_of_commissioning']); ?></div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($helicopter['Year_of_decommissioning'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Конец службы</div>
                                <div class="spec-value"><?php echo htmlspecialchars($helicopter['Year_of_decommissioning']); ?></div>
                            </div>
                        <?php endif; ?>
                        ...
                        <div class="spec-card">
                            ...
                            <div class="spec-value">
                                <span class="service-status <?php echo ($helicopter['In_service'] == 1) ? 'service-active' : 'service-inactive'; ?>">
                                    <?php echo ($helicopter['In_service'] == 1) ? 'В эксплуатации' : 'Снят с эксплуатации'; ?>
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <?php if ($can_edit_article): ?>
                <div class="edit-form-container" id="editFormContainer" style="display: none;">
                    <form method="POST" class="edit-form" id="editForm">
                        <h2 class="edit-form-title">Редактирование данных вертолета</h2>
                        
                        <div class="form-grid">
                            <div class="form-group">
                                <label class="form-label required">Название вертолета</label>
                                <input type="text" name="name" value="<?php echo htmlspecialchars($helicopter['Name'], ENT_QUOTES, 'UTF-8'); ?>" class="form-input" required>
                            </div>
                                ...
                                <select name="country_id" class="form-select" required>
                                    ...
                                    <?php foreach ($countries as $country): ?>
                                        <option value="<?php echo $country['ID']; ?>" 
                                            <?php echo $helicopter['Country_ID'] == $country['ID'] ? 'selected' : ''; ?>>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="year_commissioning" value="<?php echo !empty($helicopter['Year_of_commissioning']) ? htmlspecialchars($helicopter['Year_of_commissioning']) : ''; ?>" class="form-input" min="1900" max="2030">
                            </div>
                            
                            <div class="form-group">
                                <label class="form-label">Год снятия с эксплуатации</label>
                                <input type="number" name="year_decommissioning" value="<?php echo !empty($helicopter['Year_of_decommissioning']) ? htmlspecialchars($helicopter['Year_of_decommissioning']) : ''; ?>" class="form-input" min="1900" max="2030">
                                <div class="form-hint">Оставьте пустым, если вертолет всё ещё в эксплуатации</div>
                            </div>
                        </div>
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-grid">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="weapon_type" value="<?php echo !empty($current_weapon['Type']) ? htmlspecialchars($current_weapon['Type'], ENT_QUOTES, 'UTF-8') : ''; ?>" class="form-input" placeholder="Например: Пушка, Ракетное вооружение">
                                </div>
                                
                                <div class="form-group">
                                    <label class="form-label">Калибр</label>
                                    <input type="text" name="weapon_calibre" value="<?php echo !empty($current_weapon['Calibre']) ? htmlspecialchars($current_weapon['Calibre'], ENT_QUOTES, 'UTF-8') : ''; ?>" class="form-input" placeholder="Например: 30 мм, 7.62 мм">
                                </div>
                                
                                <div class="form-group">
                                    <label class="form-label">Название вооружения</label>
                                    <input type="text" name="weapon_name" value="<?php echo !empty($current_weapon['Name']) ? htmlspecialchars($current_weapon['Name'], ENT_QUOTES, 'UTF-8') : ''; ?>" class="form-input" placeholder="Например: 2А42, ПТУР">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="war_name" value="<?php echo !empty($current_war['Name']) ? htmlspecialchars($current_war['Name'], ENT_QUOTES, 'UTF-8') : ''; ?>" class="form-input" placeholder="Например: Афганская война, Чеченский конфликт">
                            </div>
                        </div>
                        
                        <div class="form-group full-width">
                            <label class="form-label">История и характеристики</label>
                            <textarea name="history" class="form-textarea" rows="8"><?php echo htmlspecialchars($helicopter['History'] ?? ''); ?></textarea>
                        </div>
                        
                        <div class="form-actions">
                            <button type="submit" name="update_helicopter" class="form-btn save-btn">💾 Сохранить изменения</button>
            ...
            <div class="history-section">
                ...
                <div class="history-content">
                    <?php echo nl2br(htmlspecialchars($helicopter['History'] ?? 'Описание отсутствует')); ?>
                </div>
            </div>

            <div class="action-buttons">
                <a href="<?php echo $back_url; ?>" class="back-button">← Назад к списку вертолётов</a>
            ...
            if (editForm) {
                editForm.addEventListener('submit', function(e) {
                    ...
                    if (!nameInput.value.trim()) {
                        e.preventDefault();
                        alert('Пожалуйста, введите название вертолета');
                        nameInput.focus();
                        return;
                    }
