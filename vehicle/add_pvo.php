...
// Получаем тип ПВО из параметра URL
...
$page_title = "Добавить " . mb_strtolower($pvo_type_names[$pvo_type]);
...
if (!$can_add_article) {
    header("Location: pvo.php?type=" . $pvo_type);
    exit();
}
...
// Классы (ищем ПВО - Class_ID = 2)
$class_stmt = $mysqli->prepare("SELECT ID, Name FROM Class WHERE ID = 2 ORDER BY Name");
if ($class_stmt) {
    $class_stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $generation_id = $pvo_type; // Используем переданный тип ПВО как Generation_ID
    $type_id = 2; // Устанавливаем Type_ID = 2 (наземная техника)

    // Валидация
    if (empty($name)) {
        $error_message = 'Название системы ПВО обязательно для заполнения';
        ...
        // Вставляем данные в базу с указанием типа ПВО как Generation_ID и Type_ID = 2
        ...
        $insert_stmt->bind_param("ssiiiiiii", 
            ...
            $class_id, 
            $service_id, 
            $generation_id, // Используем pvo_type как Generation_ID
        ...
        if ($insert_stmt->execute()) {
            ...
            if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                ...
                $filename = 'pvo_' . $vehicle_id . '_' . time() . '.' . $file_extension;
            ...
            $_SESSION['success_message'] = 'Система ПВО успешно добавлена!';
            
            // Перенаправляем на страницу со списком ПВО соответствующего типа
            header("Location: pvo.php?type=" . $pvo_type);
            exit();
            
        } else {
            $error_message = 'Ошибка при добавлении системы ПВО: ' . $mysqli->error;
        }
        $insert_stmt->close();
    }
}
...
<body>
    ...
    <div class="main-content">
        <div class="page-header">
            <a href="pvo.php?type=<?php echo $pvo_type; ?>" class="back-btn">← Назад к списку ПВО</a>
        </div>
        ...
        <form class="add-form" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">
            <!-- Скрытое поле для передачи типа ПВО -->
            <input type="hidden" name="type" value="<?php echo $pvo_type; ?>">
            
            <div class="form-group">
                <label class="form-label">Название системы ПВО *</label>
            ...
            <div class="form-group">
                ...
                <div class="weapon-row">
                    ...
                    <div>
                        ...
                        <input type="text" class="form-input" name="weapon_calibre" value="<?php echo htmlspecialchars($_POST['weapon_calibre'] ?? ''); ?>" placeholder="Калибр или дальность">
                    </div>
                </div>
            ...
            <div class="form-group">
                ...
                <div class="service-row">
                    ...
                    <div>
                        ...
                        <small style="color: #8b966c; font-size: 0.8rem; display: block; margin-top: 0.3rem;">Если оставить пустым, система ПВО будет отмечена как "в эксплуатации"</small>
                    </div>
                </div>
            </div>
            ...
            <button type="submit" class="submit-btn">Добавить систему ПВО</button>
        </form>
    </div>
