...
// Получаем тип бомбардировщика из параметра URL
...
$page_title = "Добавить " . mb_strtolower($bomber_type_names[$bomber_type]);
...
if (!$can_add_article) {
    header("Location: bombers.php?type=" . $bomber_type);
    exit();
}
...
// Классы (ищем бомбардировщики - Class_ID = 8)
$class_stmt = $mysqli->prepare("SELECT ID, Name FROM Class WHERE ID = 8 ORDER BY Name");
if ($class_stmt) {
    $class_stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $class_id = 8; // Бомбардировщики
    $service_start = trim($_POST['service_start'] ?? '');
    $service_end = trim($_POST['service_end'] ?? '');
    $generation_id = $bomber_type; // Используем переданный тип бомбардировщика как Generation_ID
    $type_id = 1; // Устанавливаем Type_ID = 1 по умолчанию

    // Валидация
    if (empty($name)) {
        $error_message = 'Название бомбардировщика обязательно для заполнения';
        ...
        // Вставляем данные в базу с указанием типа бомбардировщика как Generation_ID и Type_ID = 1
        ...
        $insert_stmt->bind_param("ssiiiiiii", 
            ...
            $class_id, 
            $service_id, 
            $generation_id, // Используем bomber_type как Generation_ID
        ...
        if ($insert_stmt->execute()) {
            ...
            // Логируем создание бомбардировщика - ИСПРАВЛЕНО
            $logger->logVehicleCreate($vehicle_id, $name);
            ...
            if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                ...
                $filename = 'bomber_' . $vehicle_id . '_' . time() . '.' . $file_extension;
                ...
                if (move_uploaded_file($_FILES['image']['tmp_name'], $filepath)) {
                    ...
                    // Логируем добавление изображения - ИСПРАВЛЕНО
                    $logger->logVehicleImageUpdate($vehicle_id, $name);
                }
            }
            
            // Устанавливаем сообщение об успехе
            $_SESSION['success_message'] = 'Бомбардировщик успешно добавлен!';
            
            // Перенаправляем на страницу со списком бомбардировщиков соответствующего типа
            header("Location: bombers.php?type=" . $bomber_type . "&added=1");
            exit();
            
        } else {
            $error_message = 'Ошибка при добавлении бомбардировщика: ' . $mysqli->error;
        }
        $insert_stmt->close();
    }
}
...
<body>
    ...
    <div class="main-content">
        <div class="page-header">
            <a href="bombers.php?type=<?php echo $bomber_type; ?>" class="back-btn">← Назад к списку бомбардировщиков</a>
        </div>
        ...
        <form class="add-form" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">
            <!-- Скрытое поле для передачи типа бомбардировщика -->
            <input type="hidden" name="type" value="<?php echo $bomber_type; ?>">
            
            <div class="form-group">
                <label class="form-label">Название бомбардировщика *</label>
            ...
            <div class="form-group">
                ...
                <div class="service-row">
                    ...
                    <div>
                        ...
                        <small style="color: #8b966c; font-size: 0.8rem; display: block; margin-top: 0.3rem;">Если оставить пустым, бомбардировщик будет отмечен как "в эксплуатации"</small>
                    </div>
                </div>
            </div>
            ...
            <button type="submit" class="submit-btn">Добавить бомбардировщик</button>
        </form>
    </div>
