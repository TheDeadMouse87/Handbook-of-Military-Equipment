...
// Получаем ID бомбардировщика из GET параметра
$bomber_id = isset($_GET['id']) ? intval($_GET['id']) : 0;

// Определяем страницу для кнопки "Назад" на основе параметра type
$back_url = '../vehicle/bombers.php?type=10'; // значение по умолчанию (Стратегические бомбардировщики)

if (isset($_GET['type'])) {
    $type = intval($_GET['type']);
    $back_url = '../vehicle/bombers.php?type=' . $type;
} elseif (isset($_GET['from']) && $_GET['from'] == 'bombers') {
    // Если пришли из общего списка бомбардировщиков
    $back_url = '../vehicle/bombers.php?type=10';
}
...
// Обработка обновления данных бомбардировщика
if ($can_edit_article && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_bomber'])) {
    // Получаем текущие данные бомбардировщика ДО обновления для логирования
    $old_data_stmt = $mysqli->prepare("
        ...
        WHERE v.ID = ?
    ");
    $old_data_stmt->bind_param("i", $bomber_id);
    $old_data_stmt->execute();
    $old_data_result = $old_data_stmt->get_result();
    $old_bomber_data = $old_data_result->fetch_assoc();
    $old_data_stmt->close();
    ...
    try {
        ...
        $old_values_for_log = [
            'Name' => $old_bomber_data['Name'] ?? '',
            'History' => $old_bomber_data['History'] ?? '',
            'Country_ID' => $old_bomber_data['Country_ID'] ?? 0,
            'Weapon_ID' => $old_bomber_data['Weapon_ID'] ?? null,
            'War_ID' => $old_bomber_data['War_ID'] ?? null,
            'Year_of_commissioning' => $old_bomber_data['Year_of_commissioning'] ?? 0,
            'Year_of_decommissioning' => $old_bomber_data['Year_of_decommissioning'] ?? 0,
            'In_service' => $old_bomber_data['In_service'] ?? 0
        ];
        ...
        if ($old_bomber_data) {
            $old_values_for_log['CountryName'] = $old_bomber_data['CountryName'] ?? '';
            $old_values_for_log['WeaponName'] = $old_bomber_data['WeaponName'] ?? '';
            $old_values_for_log['WeaponType'] = $old_bomber_data['WeaponType'] ?? '';
            $old_values_for_log['WeaponCalibre'] = $old_bomber_data['WeaponCalibre'] ?? '';
            $old_values_for_log['WarPeriod'] = $old_bomber_data['WarPeriod'] ?? '';
        }
        ...
        $description = "Бомбардировщик '{$old_bomber_data['Name']}' отредактирован пользователем ID: {$_SESSION['user_id']}";
        
        // Записываем лог
        $logger->logUpdate('Vehicle', $bomber_id, $old_values_for_log, $new_values_for_log, $description);
        
        $_SESSION['favorite_message'] = 'Данные бомбардировщика успешно обновлены!';
    ...
    header("Location: bomber_detail.php?" . $redirect_params);
    exit();
}
...
if (isset($_SESSION['user_id']) && isset($_POST['toggle_favorite'])) {
    ...
    $check_favorite->bind_param("ii", $user_id, $bomber_id);
    ...
    if ($check_favorite->num_rows > 0) {
        ...
        $delete_favorite->bind_param("ii", $user_id, $bomber_id);
        $delete_favorite->execute();
        $is_favorite = false;
        
        // ЛОГИРОВАНИЕ: Удаление из избранного
        $logger->log('favorite_remove', 'user_favorites', $bomber_id, 
            "Бомбардировщик '{$bomber['Name']}' удален из избранного пользователем ID: $user_id");
        ...
        $add_favorite->bind_param("ii", $user_id, $bomber_id);
        $add_favorite->execute();
        $is_favorite = true;
        
        // ЛОГИРОВАНИЕ: Добавление в избранное
        $logger->log('favorite_add', 'user_favorites', $bomber_id, 
            "Бомбардировщик '{$bomber['Name']}' добавлен в избранное пользователем ID: $user_id");
...
if ($can_edit_article && isset($_POST['upload_image'])) {
    if (isset($_FILES['new_image']) && $_FILES['new_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($file_extension, $allowed_extensions)) {
            ...
            $new_filename = 'bomber_' . $bomber_id . '_' . time() . '.' . $file_extension;
            ...
            if ($_FILES['new_image']['size'] <= 5 * 1024 * 1024) {
                if (move_uploaded_file($_FILES['new_image']['tmp_name'], $upload_path)) {
                    ...
                    $check_image->bind_param("i", $bomber_id);
                    $check_image->execute();
                    ...
                    if ($check_image->num_rows > 0) {
                        ...
                        $update_image->bind_param("ssi", $relative_path, $image_type, $bomber_id);
                        
                        if ($update_image->execute()) {
                            // ЛОГИРОВАНИЕ: Записываем обновление изображения
                            $description = "Обновлено изображение для бомбардировщика '{$bomber['Name']}'";
                            $logger->logUpdate('Vehicle', $bomber_id, 
                        ...
                        $insert_image->bind_param("iss", $bomber_id, $relative_path, $image_type);
                        
                        if ($insert_image->execute()) {
                            // ЛОГИРОВАНИЕ: Записываем загрузку изображения
                            $description = "Загружено новое изображение для бомбардировщика '{$bomber['Name']}'";
                            $logger->logUpdate('Vehicle', $bomber_id, 
    ...
    header("Location: bomber_detail.php?id=" . $bomber_id . (isset($_GET['type']) ? '&type=' . $_GET['type'] : '') . (isset($_GET['from']) ? '&from=' . $_GET['from'] : ''));
    exit();
}
...
if ($can_edit_article && isset($_POST['delete_image'])) {
    ...
    if ($check_image->num_rows > 0) {
        ...
        $description = "Удалено изображение для бомбардировщика '{$bomber['Name']}'";
        $logger->logUpdate('Vehicle', $bomber_id, 
        ...
        if ($file_deleted) {
            ...
            $delete_image->bind_param("i", $bomber_id);
...
// Получаем полную информацию о бомбардировщике
$query = "
    SELECT 
        v.ID, 
        v.Name, 
    ...
    LEFT JOIN War war ON v.War_ID = war.ID
    LEFT JOIN Service s ON v.Service_ID = s.ID
    WHERE v.ID = ? AND v.Class_ID = 8
";

$stmt = $mysqli->prepare($query);
$stmt->bind_param("i", $bomber_id);
$stmt->execute();
$result = $stmt->get_result();
$bomber = $result->fetch_assoc();
...
if ($bomber) {
    $bomber['Name'] = clean_output($bomber['Name']);
    $bomber['History'] = clean_output($bomber['History'] ?? '');
}
...
if ($bomber['Weapon_ID']) {
    ...
    $weapon_stmt->bind_param("i", $bomber['Weapon_ID']);
    $weapon_stmt->execute();
...
if ($bomber['War_ID']) {
    ...
    $war_stmt->bind_param("i", $bomber['War_ID']);
    $war_stmt->execute();
...
if (isset($_SESSION['user_id'])) {
    $check_favorite = $mysqli->prepare("SELECT id FROM user_favorites WHERE user_id = ? AND article_id = ?");
    $check_favorite->bind_param("ii", $_SESSION['user_id'], $bomber_id);
    $check_favorite->execute();
    $check_favorite->store_result();
...
if ($image_stmt) {
    $image_stmt->bind_param("i", $bomber_id);
    $image_stmt->execute();
...
<head>
    ...
    <title><?php echo htmlspecialchars($bomber['Name'], ENT_QUOTES, 'UTF-8'); ?> - Военный справочник</title>
        ...
        <div class="vehicle-detail">
            <div class="vehicle-header">
                <div class="image-section">
                    <div class="vehicle-image-large" id="vehicleImageContainer">
                        <?php if ($image_exists): ?>
                            <img src="../<?php echo htmlspecialchars($image_path); ?>" alt="<?php echo htmlspecialchars($bomber['Name'], ENT_QUOTES, 'UTF-8'); ?>" id="vehicleImage">
                        <?php else: ?>
                ...
                <div class="vehicle-info">
                    <div class="vehicle-title-section">
                        <h1 class="vehicle-name"><?php echo htmlspecialchars($bomber['Name'], ENT_QUOTES, 'UTF-8'); ?></h1>
                        <?php if ($can_edit_article): ?>
                    ...
                    <div class="vehicle-specs-grid">
                        <?php if (!empty($bomber['CountryName'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Страна</div>
                                <div class="spec-value"><?php echo htmlspecialchars($bomber['CountryName']); ?></div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($bomber['ClassName'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Класс</div>
                                <div class="spec-value"><?php echo htmlspecialchars($bomber['ClassName']); ?></div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($bomber['WeaponName']) || !empty($bomber['WeaponType']) || !empty($bomber['WeaponCalibre'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Вооружение</div>
                                <div class="spec-value">
                                    <?php if (!empty($bomber['WeaponName'])): ?>
                                        <?php echo htmlspecialchars($bomber['WeaponName'], ENT_QUOTES, 'UTF-8'); ?>
                                    <?php endif; ?>
                                </div>
                                <div class="weapon-details">
                                    <?php if (!empty($bomber['WeaponType'])): ?>
                                        <div class="weapon-detail">Тип: <?php echo htmlspecialchars($bomber['WeaponType'], ENT_QUOTES, 'UTF-8'); ?></div>
                                    <?php endif; ?>
                                    <?php if (!empty($bomber['WeaponCalibre'])): ?>
                                        <div class="weapon-detail">Калибр/Масса: <?php echo htmlspecialchars($bomber['WeaponCalibre'], ENT_QUOTES, 'UTF-8'); ?></div>
                                    <?php endif; ?>
                                </div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($bomber['WarPeriod'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Участия в войнах или конфликтах</div>
                                <div class="spec-value"><?php echo htmlspecialchars($bomber['WarPeriod'], ENT_QUOTES, 'UTF-8'); ?></div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($bomber['Year_of_commissioning'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Начало службы</div>
                                <div class="spec-value"><?php echo htmlspecialchars($bomber['Year_of_commissioning']); ?></div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($bomber['Year_of_decommissioning'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Конец службы</div>
                                <div class="spec-value"><?php echo htmlspecialchars($bomber['Year_of_decommissioning']); ?></div>
                            </div>
                        <?php endif; ?>
                        ...
                        <div class="spec-card">
                            ...
                            <div class="spec-value">
                                <span class="service-status <?php echo ($bomber['In_service'] == 1) ? 'service-active' : 'service-inactive'; ?>">
                                    <?php echo ($bomber['In_service'] == 1) ? 'В эксплуатации' : 'Снят с эксплуатации'; ?>
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <?php if ($can_edit_article): ?>
                <div class="edit-form-container" id="editFormContainer" style="display: none;">
                    <form method="POST" class="edit-form" id="editForm">
                        <h2 class="edit-form-title">Редактирование данных бомбардировщика</h2>
                        
                        <div class="form-grid">
                            <div class="form-group">
                                <label class="form-label required">Название бомбардировщика</label>
                                <input type="text" name="name" value="<?php echo htmlspecialchars($bomber['Name'], ENT_QUOTES, 'UTF-8'); ?>" class="form-input" required>
                            </div>
                                ...
                                <select name="country_id" class="form-select" required>
                                    ...
                                    <?php foreach ($countries as $country): ?>
                                        <option value="<?php echo $country['ID']; ?>" 
                                            <?php echo $bomber['Country_ID'] == $country['ID'] ? 'selected' : ''; ?>>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="year_commissioning" value="<?php echo !empty($bomber['Year_of_commissioning']) ? htmlspecialchars($bomber['Year_of_commissioning']) : ''; ?>" class="form-input" min="1900" max="2030">
                            </div>
                            
                            <div class="form-group">
                                <label class="form-label">Год снятия с эксплуатации</label>
                                <input type="number" name="year_decommissioning" value="<?php echo !empty($bomber['Year_of_decommissioning']) ? htmlspecialchars($bomber['Year_of_decommissioning']) : ''; ?>" class="form-input" min="1900" max="2030">
                                <div class="form-hint">Оставьте пустым, если бомбардировщик всё ещё в эксплуатации</div>
                            </div>
                        </div>
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-grid">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="weapon_type" value="<?php echo !empty($current_weapon['Type']) ? htmlspecialchars($current_weapon['Type'], ENT_QUOTES, 'UTF-8') : ''; ?>" class="form-input" placeholder="Например: Бомбовое вооружение, Ракетное вооружение">
                                </div>
                                
                                <div class="form-group">
                                    <label class="form-label">Калибр/Масса</label>
                                    <input type="text" name="weapon_calibre" value="<?php echo !empty($current_weapon['Calibre']) ? htmlspecialchars($current_weapon['Calibre'], ENT_QUOTES, 'UTF-8') : ''; ?>" class="form-input" placeholder="Например: 500 кг, 9 тонн">
                                </div>
                                
                                <div class="form-group">
                                    <label class="form-label">Название вооружения</label>
                                    <input type="text" name="weapon_name" value="<?php echo !empty($current_weapon['Name']) ? htmlspecialchars($current_weapon['Name'], ENT_QUOTES, 'UTF-8') : ''; ?>" class="form-input" placeholder="Например: ФАБ-500, Х-101">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group full-width">
                            ...
                            <textarea name="history" class="form-textarea" rows="8"><?php echo htmlspecialchars($bomber['History'] ?? ''); ?></textarea>
                        </div>
                        
                        <div class="form-actions">
                            <button type="submit" name="update_bomber" class="form-btn save-btn">💾 Сохранить изменения</button>
            ...
            <div class="history-section">
                ...
                <div class="history-content">
                    <?php echo nl2br(htmlspecialchars($bomber['History'] ?? 'Описание отсутствует')); ?>
                </div>
            </div>

            <div class="action-buttons">
                <a href="<?php echo $back_url; ?>" class="back-button">← Назад к списку бомбардировщиков</a>
            ...
            if (editForm) {
                editForm.addEventListener('submit', function(e) {
                    ...
                    if (!nameInput.value.trim()) {
                        e.preventDefault();
                        alert('Пожалуйста, введите название бомбардировщика');
                        nameInput.focus();
                        return;
                    }
