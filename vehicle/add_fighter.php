...
// Получаем тип истребителя из параметра URL
$fighter_type = isset($_GET['type']) ? intval($_GET['type']) : 2;
$fighter_type = in_array($fighter_type, [2, 3, 4, 5]) ? $fighter_type : 2;

// Названия типов истребителей для отображения
$fighter_type_names = [
    2 => 'Истребители-перехватчики',
    3 => 'Многоцелевые истребители', 
    4 => 'Палубные истребители',
    5 => 'Фронтовые истребители'
];

$page_title = "Добавить " . mb_strtolower($fighter_type_names[$fighter_type]);
...
if (!$can_add_article) {
    header("Location: fighters.php?type=" . $fighter_type);
    exit();
}
...
// Классы (ищем истребители - Class_ID = 6)
$class_stmt = $mysqli->prepare("SELECT ID, Name FROM Class WHERE ID = 6 ORDER BY Name");
if ($class_stmt) {
    $class_stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $class_id = 6; // Истребители
    $service_start = trim($_POST['service_start'] ?? '');
    $service_end = trim($_POST['service_end'] ?? '');
    $generation_id = $fighter_type; // Используем переданный тип истребителя как Generation_ID
    $type_id = 1; // Устанавливаем Type_ID = 1 по умолчанию

    // Валидация
    if (empty($name)) {
        $error_message = 'Название истребителя обязательно для заполнения';
        ...
        // Вставляем данные в базу с указанием типа истребителя как Generation_ID и Type_ID = 1
        ...
        $insert_stmt->bind_param("ssiiiiiii", 
            ...
            $class_id, 
            $service_id, 
            $generation_id, // Используем fighter_type как Generation_ID
        ...
        if ($insert_stmt->execute()) {
            ...
            // Логируем создание истребителя
            $logger->logCreate('Vehicle', $vehicle_id, "Добавлен истребитель: " . $name);
            ...
            if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                ...
                $filename = 'fighter_' . $vehicle_id . '_' . time() . '.' . $file_extension;
                ...
                if (move_uploaded_file($_FILES['image']['tmp_name'], $filepath)) {
                    ...
                    $logger->logCreate('vehicle_images', $mysqli->insert_id, "Добавлено изображение для истребителя: " . $name);
                }
            }
            
            // Устанавливаем сообщение об успехе
            $_SESSION['success_message'] = 'Истребитель успешно добавлен!';
            
            // Перенаправляем на страницу со списком истребителей соответствующего типа
            header("Location: fighters.php?type=" . $fighter_type . "&added=1");
            exit();
            
        } else {
            $error_message = 'Ошибка при добавлении истребителя: ' . $mysqli->error;
        }
        $insert_stmt->close();
    }
}
...
<body>
    ...
    <div class="main-content">
        <div class="page-header">
            <a href="fighters.php?type=<?php echo $fighter_type; ?>" class="back-btn">← Назад к списку истребителей</a>
        </div>
        ...
        <form class="add-form" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">
            <!-- Скрытое поле для передачи типа истребителя -->
            <input type="hidden" name="type" value="<?php echo $fighter_type; ?>">
            
            <div class="form-group">
                <label class="form-label">Название истребителя *</label>
            ...
            <div class="form-group">
                ...
                <div class="service-row">
                    ...
                    <div>
                        ...
                        <small style="color: #8b966c; font-size: 0.8rem; display: block; margin-top: 0.3rem;">Если оставить пустым, истребитель будет отмечен как "в эксплуатации"</small>
                    </div>
                </div>
            </div>
            ...
            <button type="submit" class="submit-btn">Добавить истребитель</button>
        </form>
    </div>
