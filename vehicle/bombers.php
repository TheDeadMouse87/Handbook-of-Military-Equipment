...
// Определяем какой тип бомбардировщиков отображать из параметра URL
$bomber_type = isset($_GET['type']) ? intval($_GET['type']) : 10;
$bomber_type = in_array($bomber_type, [10, 11, 12, 13]) ? $bomber_type : 10;

// Названия типов бомбардировщиков для отображения
$bomber_type_names = [
    10 => 'Стратегические бомбардировщики',
    11 => 'Тактические бомбардировщики', 
    12 => 'Штурмовики-бомбардировщики',
    13 => 'Истребители-бомбардировщики'
];

$page_title = $bomber_type_names[$bomber_type];
...
$sql = "
    ...
    WHERE v.Class_ID = 8 AND v.Generation_ID = ?
";
...
// Обработка удаления бомбардировщика
if (isset($_POST['delete_bomber']) && $can_delete_article) {
    $bomber_id = intval($_POST['bomber_id']);
    
    // Получаем информацию о бомбардировщике для лога
    $bomber_info_stmt = $mysqli->prepare("SELECT Name FROM Vehicle WHERE ID = ?");
    $bomber_info_stmt->bind_param("i", $bomber_id);
    $bomber_info_stmt->execute();
    $bomber_info_result = $bomber_info_stmt->get_result();
    $bomber_info = $bomber_info_result->fetch_assoc();
    $bomber_info_stmt->close();
    ...
    try {
        ...
        $delete_images_stmt->bind_param("i", $bomber_id);
        $delete_images_stmt->execute();
        $delete_images_stmt->close();
        
        // Удаляем сам бомбардировщик
        $delete_bomber_stmt = $mysqli->prepare("DELETE FROM Vehicle WHERE ID = ?");
        $delete_bomber_stmt->bind_param("i", $bomber_id);
        $delete_bomber_stmt->execute();
        $delete_bomber_stmt->close();
        ...
        // Логируем удаление
        if ($bomber_info) {
            $logger->logDelete('Vehicle', $bomber_id, "Удален бомбардировщик: " . $bomber_info['Name']);
        }
        
        // Редирект для обновления страницы
        header("Location: bombers.php?type=" . $bomber_type . "&deleted=1");
        exit();
        
    } catch (Exception $e) {
        // Откатываем транзакцию в случае ошибки
        $mysqli->rollback();
        $delete_error = "Ошибка при удалении бомбардировщика: " . $e->getMessage();
    }
}

// Получаем количество бомбардировщиков для отображения
$bombers_count = is_array($bombers) ? count($bombers) : 0;
?>
...
<body>
    ...
    <div class="main-content">
        <div class="page-header">
            ...
            <?php if ($can_add_article): ?>
                <div class="add-btn-container">
                    <a href="add_bomber.php?type=<?php echo $bomber_type; ?>" class="add-btn">+ Добавить бомбардировщик</a>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="filter-section" id="filterSection">
            <form method="GET" class="filter-form" id="filterForm">
                <input type="hidden" name="type" value="<?php echo $bomber_type; ?>">
        ...
        <div class="results-count">
            Найдено бомбардировщиков: <?php echo $bombers_count; ?>
        </div>

        <?php if ($bombers_count === 0): ?>
            <div class="no-vehicles">
                <p><?php echo $bomber_type_names[$bomber_type]; ?> не найдены</p>
                <?php if ($filter_country > 0 || $filter_service || $filter_year_from > 0 || $filter_year_to > 0): ?>
        ...
        <?php else: ?>
            <div class="vehicle-list">
                <?php foreach ($bombers as $bomber): ?>
                    <div class="vehicle-item">
                        <?php if (!empty($bomber['ImagePath'])): ?>
                            <img src="../<?php echo htmlspecialchars($bomber['ImagePath']); ?>"
                                 alt="<?php echo htmlspecialchars($bomber['Name']); ?>" 
                                 class="vehicle-image">
                        <?php else: ?>
                        ...
                        <div class="vehicle-content">
                            <a href="bomber_detail.php?id=<?php echo $bomber['ID']; ?>&from=bombers&type=<?php echo $bomber_type; ?>" class="vehicle-name-link">
                                <h3 class="vehicle-name"><?php echo htmlspecialchars($bomber['Name']); ?></h3>
                            </a>
                            ...
                            <div class="vehicle-details">
                                <?php if (!empty($bomber['CountryName'])): ?>
                                    <div class="detail-group">
                                        ...
                                        <span class="detail-value"><?php echo htmlspecialchars($bomber['CountryName']); ?></span>
                                    </div>
                                <?php endif; ?>
                                
                                <?php if (!empty($bomber['TypeName'])): ?>
                                    <div class="detail-group">
                                        <span class="detail-label">Тип:</span>
                                        <span class="detail-value"><?php echo htmlspecialchars($bomber['TypeName']); ?></span>
                                    </div>
                                <?php endif; ?>
                                ...
                                <?php if (!empty($bomber['WeaponName'])): ?>
                                    <div class="detail-group">
                                        ...
                                        <span class="detail-value"><?php echo htmlspecialchars($bomber['WeaponName']); ?></span>
                                    </div>
                                <?php endif; ?>
                                ...
                                <?php if (!empty($bomber['Year_of_commissioning'])): ?>
                                    <div class="detail-group">
                                        ...
                                        <span class="detail-value"><?php echo htmlspecialchars($bomber['Year_of_commissioning']); ?></span>
                                    </div>
                                <?php endif; ?>
                            ...
                            <?php if ($can_delete_article): ?>
                                <div class="vehicle-actions">
                                    <button type="button" class="delete-btn" 
                                            onclick="showDeleteModal(<?php echo $bomber['ID']; ?>, '<?php echo htmlspecialchars(addslashes($bomber['Name'])); ?>')">
                                        Удалить
                                    </button>
                                </div>
                            <?php endif; ?>
    ...
    <div class="modal-overlay" id="deleteModal">
        <div class="modal-content">
            ...
            <p class="modal-text" id="deleteModalText">
                Вы действительно хотите удалить бомбардировщик?
            </p>
            <div class="modal-actions">
                <form method="POST" id="deleteForm">
                    <input type="hidden" name="bomber_id" id="deleteBomberId">
                    <input type="hidden" name="delete_bomber" value="1">
    ...
    <script>
        ...
        function resetFilters() {
            ...
            url.search = '?type=' + <?php echo $bomber_type; ?>;
        ...
        function showDeleteModal(bomberId, bomberName) {
            const modal = document.getElementById('deleteModal');
            const modalText = document.getElementById('deleteModalText');
            const deleteForm = document.getElementById('deleteForm');
            const deleteBomberId = document.getElementById('deleteBomberId');
            
            modalText.textContent = `Вы действительно хотите удалить бомбардировщик "${bomberName}"? Это действие нельзя отменить.`;
            deleteBomberId.value = bomberId;
