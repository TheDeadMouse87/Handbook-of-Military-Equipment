...
// Получаем ID танка из GET параметра
$tank_id = isset($_GET['id']) ? intval($_GET['id']) : 0;

// Определяем страницу для кнопки "Назад" на основе параметра type
$back_url = '../vehicle/tanks.php?type=14'; // значение по умолчанию (легкие танки)

if (isset($_GET['type'])) {
    $type = intval($_GET['type']);
    $back_url = '../vehicle/tanks.php?type=' . $type;
} elseif (isset($_GET['from']) && $_GET['from'] == 'tanks') {
    // Если пришли из общего списка танков
    $back_url = '../vehicle/tanks.php?type=14';
}
...
// Обработка обновления данных танка
if ($can_edit_article && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_tank'])) {
    // Получаем текущие данные танка ДО обновления для логирования
    $old_data_stmt = $mysqli->prepare("
        ...
        WHERE v.ID = ?
    ");
    $old_data_stmt->bind_param("i", $tank_id);
    $old_data_stmt->execute();
    $old_data_result = $old_data_stmt->get_result();
    $old_tank_data = $old_data_result->fetch_assoc();
    $old_data_stmt->close();
    ...
    try {
        ...
        $old_values_for_log = [
            'Name' => $old_tank_data['Name'] ?? '',
            'History' => $old_tank_data['History'] ?? '',
            'Country_ID' => $old_tank_data['Country_ID'] ?? 0,
            'Weapon_ID' => $old_tank_data['Weapon_ID'] ?? null,
            'War_ID' => $old_tank_data['War_ID'] ?? null,
            'Year_of_commissioning' => $old_tank_data['Year_of_commissioning'] ?? 0,
            'Year_of_decommissioning' => $old_tank_data['Year_of_decommissioning'] ?? 0,
            'In_service' => $old_tank_data['In_service'] ?? 0
        ];
        ...
        if ($old_tank_data) {
            $old_values_for_log['CountryName'] = $old_tank_data['CountryName'] ?? '';
            $old_values_for_log['WeaponName'] = $old_tank_data['WeaponName'] ?? '';
            $old_values_for_log['WeaponType'] = $old_tank_data['WeaponType'] ?? '';
            $old_values_for_log['WeaponCalibre'] = $old_tank_data['WeaponCalibre'] ?? '';
            $old_values_for_log['WarPeriod'] = $old_tank_data['WarPeriod'] ?? '';
        }
        ...
        $description = "Танк '{$old_tank_data['Name']}' отредактирован пользователем ID: {$_SESSION['user_id']}";
        
        // Записываем лог
        $logger->logUpdate('Vehicle', $tank_id, $old_values_for_log, $new_values_for_log, $description);
        
        $_SESSION['favorite_message'] = 'Данные танка успешно обновлены!';
    ...
    header("Location: tank_detail.php?" . $redirect_params);
    exit();
}
...
if (isset($_SESSION['user_id']) && isset($_POST['toggle_favorite'])) {
    ...
    $check_favorite->bind_param("ii", $user_id, $tank_id);
    ...
    if ($check_favorite->num_rows > 0) {
        ...
        $delete_favorite->bind_param("ii", $user_id, $tank_id);
        $delete_favorite->execute();
        $is_favorite = false;
        
        // ЛОГИРОВАНИЕ: Удаление из избранного
        $logger->log('favorite_remove', 'user_favorites', $tank_id, 
            "Танк '{$tank['Name']}' удален из избранного пользователем ID: $user_id");
        ...
        $add_favorite->bind_param("ii", $user_id, $tank_id);
        $add_favorite->execute();
        $is_favorite = true;
        
        // ЛОГИРОВАНИЕ: Добавление в избранное
        $logger->log('favorite_add', 'user_favorites', $tank_id, 
            "Танк '{$tank['Name']}' добавлен в избранное пользователем ID: $user_id");
...
if ($can_edit_article && isset($_POST['upload_image'])) {
    if (isset($_FILES['new_image']) && $_FILES['new_image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($file_extension, $allowed_extensions)) {
            ...
            $new_filename = 'vehicle_' . $tank_id . '_' . time() . '.' . $file_extension;
            ...
            if ($_FILES['new_image']['size'] <= 5 * 1024 * 1024) {
                if (move_uploaded_file($_FILES['new_image']['tmp_name'], $upload_path)) {
                    ...
                    $check_image->bind_param("i", $tank_id);
                    $check_image->execute();
                    ...
                    if ($check_image->num_rows > 0) {
                        ...
                        $update_image->bind_param("ssi", $relative_path, $image_type, $tank_id);
                        
                        if ($update_image->execute()) {
                            // ЛОГИРОВАНИЕ: Записываем обновление изображения
                            $description = "Обновлено изображение для танка '{$tank['Name']}'";
                            $logger->logUpdate('Vehicle', $tank_id, 
                        ...
                        $insert_image->bind_param("iss", $tank_id, $relative_path, $image_type);
                        
                        if ($insert_image->execute()) {
                            // ЛОГИРОВАНИЕ: Записываем загрузку изображения
                            $description = "Загружено новое изображение для танка '{$tank['Name']}'";
                            $logger->logUpdate('Vehicle', $tank_id, 
    ...
    header("Location: tank_detail.php?id=" . $tank_id . (isset($_GET['type']) ? '&type=' . $_GET['type'] : '') . (isset($_GET['from']) ? '&from=' . $_GET['from'] : ''));
    exit();
}
...
if ($can_edit_article && isset($_POST['delete_image'])) {
    ...
    if ($check_image->num_rows > 0) {
        ...
        $description = "Удалено изображение для танка '{$tank['Name']}'";
        $logger->logUpdate('Vehicle', $tank_id, 
        ...
        if ($file_deleted) {
            ...
            $delete_image->bind_param("i", $tank_id);
...
// Получаем полную информацию о танке
$query = "
    SELECT 
        v.ID, 
        v.Name, 
...
if ($tank) {
    $tank['Name'] = clean_output($tank['Name']);
    $tank['History'] = clean_output($tank['History'] ?? '');
}
...
if ($tank['Weapon_ID']) {
    ...
    $weapon_stmt->bind_param("i", $tank['Weapon_ID']);
    $weapon_stmt->execute();
...
if ($tank['War_ID']) {
    ...
    $war_stmt->bind_param("i", $tank['War_ID']);
    $war_stmt->execute();
...
if (isset($_SESSION['user_id'])) {
    $check_favorite = $mysqli->prepare("SELECT id FROM user_favorites WHERE user_id = ? AND article_id = ?");
    $check_favorite->bind_param("ii", $_SESSION['user_id'], $tank_id);
    $check_favorite->execute();
    $check_favorite->store_result();
...
if ($image_stmt) {
    $image_stmt->bind_param("i", $tank_id);
    $image_stmt->execute();
...
<head>
    ...
    <title><?php echo htmlspecialchars($tank['Name'], ENT_QUOTES, 'UTF-8'); ?> - Военный справочник</title>
        ...
        <div class="vehicle-detail">
            <div class="vehicle-header">
                <div class="image-section">
                    <div class="vehicle-image-large" id="vehicleImageContainer">
                        <?php if ($image_exists): ?>
                            <img src="../<?php echo htmlspecialchars($image_path); ?>" alt="<?php echo htmlspecialchars($tank['Name'], ENT_QUOTES, 'UTF-8'); ?>" id="vehicleImage">
                        <?php else: ?>
                ...
                <div class="vehicle-info">
                    <div class="vehicle-title-section">
                        <h1 class="vehicle-name"><?php echo htmlspecialchars($tank['Name'], ENT_QUOTES, 'UTF-8'); ?></h1>
                        <?php if ($can_edit_article): ?>
                    ...
                    <div class="vehicle-specs-grid">
                        <?php if (!empty($tank['CountryName'])): ?>
                            <div class="spec-card">
                                ...
                                <div class="spec-value"><?php echo htmlspecialchars($tank['CountryName']); ?></div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($tank['ClassName'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Класс</div>
                                <div class="spec-value"><?php echo htmlspecialchars($tank['ClassName']); ?></div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($tank['WeaponName']) || !empty($tank['WeaponType']) || !empty($tank['WeaponCalibre'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Вооружение</div>
                                <div class="spec-value">
                                    <?php if (!empty($tank['WeaponName'])): ?>
                                        <?php echo htmlspecialchars($tank['WeaponName'], ENT_QUOTES, 'UTF-8'); ?>
                                    <?php endif; ?>
                                </div>
                                <div class="weapon-details">
                                    <?php if (!empty($tank['WeaponType'])): ?>
                                        <div class="weapon-detail">Тип: <?php echo htmlspecialchars($tank['WeaponType'], ENT_QUOTES, 'UTF-8'); ?></div>
                                    <?php endif; ?>
                                    <?php if (!empty($tank['WeaponCalibre'])): ?>
                                        <div class="weapon-detail">Калибр: <?php echo htmlspecialchars($tank['WeaponCalibre'], ENT_QUOTES, 'UTF-8'); ?></div>
                                    <?php endif; ?>
                                </div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($tank['WarPeriod'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Участия в войнах или конфликтах</div>
                                <div class="spec-value"><?php echo htmlspecialchars($tank['WarPeriod'], ENT_QUOTES, 'UTF-8'); ?></div>
                            </div>
                        <?php endif; ?>
                        ...
                        <?php if (!empty($tank['Year_of_commissioning'])): ?>
                            <div class="spec-card">
                                ...
                                <div class="spec-value"><?php echo htmlspecialchars($tank['Year_of_commissioning']); ?></div>
                            </div>
                        <?php endif; ?>
                        
                        <?php if (!empty($tank['Year_of_decommissioning'])): ?>
                            <div class="spec-card">
                                <div class="spec-label">Конец службы</div>
                                <div class="spec-value"><?php echo htmlspecialchars($tank['Year_of_decommissioning']); ?></div>
                            </div>
                        <?php endif; ?>
            ...
            <?php if ($can_edit_article): ?>
                <div class="edit-form-container" id="editFormContainer" style="display: none;">
                    <form method="POST" class="edit-form" id="editForm">
                        <h2 class="edit-form-title">Редактирование данных танка</h2>
                        
                        <div class="form-grid">
                            <div class="form-group">
                                <label class="form-label required">Название танка</label>
                                <input type="text" name="name" value="<?php echo htmlspecialchars($tank['Name'], ENT_QUOTES, 'UTF-8'); ?>" class="form-input" required>
                            </div>
                                ...
                                <select name="country_id" class="form-select" required>
                                    ...
                                    <?php foreach ($countries as $country): ?>
                                        <option value="<?php echo $country['ID']; ?>" 
                                            <?php echo $tank['Country_ID'] == $country['ID'] ? 'selected' : ''; ?>>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="year_commissioning" value="<?php echo !empty($tank['Year_of_commissioning']) ? htmlspecialchars($tank['Year_of_commissioning']) : ''; ?>" class="form-input" min="1900" max="2030">
                            </div>
                            
                            <div class="form-group">
                                <label class="form-label">Год снятия с эксплуатации</label>
                                <input type="number" name="year_decommissioning" value="<?php echo !empty($tank['Year_of_decommissioning']) ? htmlspecialchars($tank['Year_of_decommissioning']) : ''; ?>" class="form-input" min="1900" max="2030">
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-grid">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="weapon_type" value="<?php echo !empty($current_weapon['Type']) ? htmlspecialchars($current_weapon['Type'], ENT_QUOTES, 'UTF-8') : ''; ?>" class="form-input" placeholder="Например: Пушка, Пулемет">
                                </div>
                                
                                <div class="form-group">
                                    <label class="form-label">Калибр</label>
                                    <input type="text" name="weapon_calibre" value="<?php echo !empty($current_weapon['Calibre']) ? htmlspecialchars($current_weapon['Calibre'], ENT_QUOTES, 'UTF-8') : ''; ?>" class="form-input" placeholder="Например: 125 мм, 7.62 мм">
                                </div>
                                
                                <div class="form-group">
                                    <label class="form-label">Название вооружения</label>
                                    <input type="text" name="weapon_name" value="<?php echo !empty($current_weapon['Name']) ? htmlspecialchars($current_weapon['Name'], ENT_QUOTES, 'UTF-8') : ''; ?>" class="form-input" placeholder="Например: 2А46, Д-81ТМ">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group full-width">
                            ...
                            <textarea name="history" class="form-textarea" rows="8"><?php echo htmlspecialchars($tank['History'] ?? ''); ?></textarea>
                        </div>
                        
                        <div class="form-actions">
                            <button type="submit" name="update_tank" class="form-btn save-btn">💾 Сохранить изменения</button>
            ...
            <div class="history-section">
                ...
                <div class="history-content">
                    <?php echo nl2br(htmlspecialchars($tank['History'] ?? 'Описание отсутствует')); ?>
                </div>
            </div>

            <div class="action-buttons">
                <a href="<?php echo $back_url; ?>" class="back-button">← Назад к списку танков</a>
            ...
            if (editForm) {
                editForm.addEventListener('submit', function(e) {
                    ...
                    if (!nameInput.value.trim()) {
                        e.preventDefault();
                        alert('Пожалуйста, введите название танка');
                        nameInput.focus();
                        return;
                    }
