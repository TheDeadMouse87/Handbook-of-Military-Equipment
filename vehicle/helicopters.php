...
// Определяем какой тип вертолётов отображать из параметра URL
...
$page_title = $helicopter_type_names[$helicopter_type];
...
$sql = "
    ...
    WHERE v.Class_ID = 7 AND v.Generation_ID = ?
";

$params = [$helicopter_type];
$types = "i";
...
// Обработка удаления вертолета
if (isset($_POST['delete_helicopter']) && $can_delete_article) {
    $helicopter_id = intval($_POST['helicopter_id']);
    
    // Получаем информацию о вертолете для лога
    $helicopter_info_stmt = $mysqli->prepare("SELECT Name FROM Vehicle WHERE ID = ?");
    $helicopter_info_stmt->bind_param("i", $helicopter_id);
    $helicopter_info_stmt->execute();
    $helicopter_info_result = $helicopter_info_stmt->get_result();
    $helicopter_info = $helicopter_info_result->fetch_assoc();
    $helicopter_info_stmt->close();
    ...
    try {
        ...
        $delete_images_stmt->bind_param("i", $helicopter_id);
        $delete_images_stmt->execute();
        $delete_images_stmt->close();
        
        // Удаляем сам вертолет
        $delete_helicopter_stmt = $mysqli->prepare("DELETE FROM Vehicle WHERE ID = ?");
        $delete_helicopter_stmt->bind_param("i", $helicopter_id);
        $delete_helicopter_stmt->execute();
        $delete_helicopter_stmt->close();
        ...
        // Логируем удаление
        if ($helicopter_info) {
            $logger->logDelete('Vehicle', $helicopter_id, "Удален вертолет: " . $helicopter_info['Name']);
        }
        
        // Редирект для обновления страницы
        header("Location: helicopters.php?type=" . $helicopter_type . "&deleted=1");
        exit();
        
    } catch (Exception $e) {
        // Откатываем транзакцию в случае ошибки
        $mysqli->rollback();
        $delete_error = "Ошибка при удалении вертолета: " . $e->getMessage();
    }
}

// Получаем количество вертолетов для отображения
$helicopters_count = is_array($helicopters) ? count($helicopters) : 0;
?>
...
<body>
    ...
    <div class="main-content">
        <div class="page-header">
            ...
            <?php if ($can_add_article): ?>
                <div class="add-btn-container">
                    <a href="add_helicopter.php?type=<?php echo $helicopter_type; ?>" class="add-btn">+ Добавить вертолёт</a>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="results-count">
            Найдено вертолётов: <?php echo $helicopters_count; ?>
        </div>

        <?php if ($helicopters_count === 0): ?>
            <div class="no-vehicles">
                <p><?php echo $helicopter_type_names[$helicopter_type]; ?> не найдены</p>
                <?php if ($filter_country > 0 || $filter_service || $filter_year_from > 0 || $filter_year_to > 0): ?>
        ...
        <?php else: ?>
            <div class="vehicle-list">
                <?php foreach ($helicopters as $helicopter): ?>
                    <div class="vehicle-item">
                        <?php if (!empty($helicopter['ImagePath'])): ?>
                            <img src="../<?php echo htmlspecialchars($helicopter['ImagePath']); ?>"
                                 alt="<?php echo htmlspecialchars($helicopter['Name']); ?>" 
                                 class="vehicle-image">
                        <?php else: ?>
                        ...
                        <div class="vehicle-content">
                            <a href="helicopter_detail.php?id=<?php echo $helicopter['ID']; ?>&from=helicopters&type=<?php echo $helicopter_type; ?>" class="vehicle-name-link">
                                <h3 class="vehicle-name"><?php echo htmlspecialchars($helicopter['Name']); ?></h3>
                            </a>
                            ...
                            <div class="vehicle-details">
                                <?php if (!empty($helicopter['CountryName'])): ?>
                                    <div class="detail-group">
                                        ...
                                        <span class="detail-value"><?php echo htmlspecialchars($helicopter['CountryName']); ?></span>
                                    </div>
                                <?php endif; ?>
                                
                                <?php if (!empty($helicopter['TypeName'])): ?>
                                    <div class="detail-group">
                                        <span class="detail-label">Тип:</span>
                                        <span class="detail-value"><?php echo htmlspecialchars($helicopter['TypeName']); ?></span>
                                    </div>
                                <?php endif; ?>
                                ...
                                <?php if (!empty($helicopter['WeaponName'])): ?>
                                    <div class="detail-group">
                                        ...
                                        <span class="detail-value"><?php echo htmlspecialchars($helicopter['WeaponName']); ?></span>
                                    </div>
                                <?php endif; ?>
                                ...
                                <?php if (!empty($helicopter['Year_of_commissioning'])): ?>
                                    <div class="detail-group">
                                        ...
                                        <span class="detail-value"><?php echo htmlspecialchars($helicopter['Year_of_commissioning']); ?></span>
                                    </div>
                                <?php endif; ?>
                            ...
                            <?php if ($can_delete_article): ?>
                                <div class="vehicle-actions">
                                    <button type="button" class="delete-btn" 
                                            onclick="showDeleteModal(<?php echo $helicopter['ID']; ?>, '<?php echo htmlspecialchars(addslashes($helicopter['Name'])); ?>')">
                                        Удалить
                                    </button>
                                </div>
                            <?php endif; ?>
    ...
    <div class="modal-overlay" id="deleteModal">
        <div class="modal-content">
            ...
            <p class="modal-text" id="deleteModalText">
                Вы действительно хотите удалить вертолет?
            </p>
            <div class="modal-actions">
                <form method="POST" id="deleteForm">
                    <input type="hidden" name="helicopter_id" id="deleteHelicopterId">
                    <input type="hidden" name="delete_helicopter" value="1">
    ...
    <script>
        ...
        function resetFilters() {
            ...
            url.search = '?type=' + <?php echo $helicopter_type; ?>;
        ...
        function showDeleteModal(helicopterId, helicopterName) {
            const modal = document.getElementById('deleteModal');
            const modalText = document.getElementById('deleteModalText');
            const deleteForm = document.getElementById('deleteForm');
            const deleteHelicopterId = document.getElementById('deleteHelicopterId');
            
            modalText.textContent = `Вы действительно хотите удалить вертолет "${helicopterName}"? Это действие нельзя отменить.`;
            deleteHelicopterId.value = helicopterId;
