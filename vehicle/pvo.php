...
// Определяем какой тип ПВО отображать из параметра URL
$pvo_type = isset($_GET['type']) ? intval($_GET['type']) : 19;
$pvo_type = in_array($pvo_type, [19, 20, 21]) ? $pvo_type : 19;

// Названия типов ПВО для отображения
$pvo_type_names = [
    19 => 'ЗРК и ЗРПК',
    20 => 'Переносные зенитные ракетные комплексы', 
    21 => 'Зенитная артиллерия'
];

$page_title = $pvo_type_names[$pvo_type];
...
$sql = "
    ...
    WHERE v.Class_ID = 2 AND v.Generation_ID = ?
";
...
// Обработка удаления системы ПВО
if (isset($_POST['delete_pvo']) && $can_delete_article) {
    $pvo_id = intval($_POST['pvo_id']);
    
    // Получаем информацию о системе для лога
    $pvo_info_stmt = $mysqli->prepare("SELECT Name FROM Vehicle WHERE ID = ?");
    $pvo_info_stmt->bind_param("i", $pvo_id);
    $pvo_info_stmt->execute();
    $pvo_info_result = $pvo_info_stmt->get_result();
    $pvo_info = $pvo_info_result->fetch_assoc();
    $pvo_info_stmt->close();
    ...
    try {
        ...
        $delete_images_stmt->bind_param("i", $pvo_id);
        $delete_images_stmt->execute();
        $delete_images_stmt->close();
        
        // Удаляем саму систему
        $delete_pvo_stmt = $mysqli->prepare("DELETE FROM Vehicle WHERE ID = ?");
        $delete_pvo_stmt->bind_param("i", $pvo_id);
        $delete_pvo_stmt->execute();
        $delete_pvo_stmt->close();
        ...
        // Логируем удаление
        if ($pvo_info) {
            $logger->logDelete('Vehicle', $pvo_id, "Удалена система ПВО: " . $pvo_info['Name']);
        }
        
        // Редирект для обновления страницы
        header("Location: pvo.php?type=" . $pvo_type . "&deleted=1");
        exit();
        
    } catch (Exception $e) {
        // Откатываем транзакцию в случае ошибки
        $mysqli->rollback();
        $delete_error = "Ошибка при удалении системы ПВО: " . $e->getMessage();
    }
}

// Получаем количество систем ПВО для отображения
$pvo_count = is_array($pvo_systems) ? count($pvo_systems) : 0;
?>
...
<body>
    ...
    <div class="main-content">
        <div class="page-header">
            ...
            <?php if ($can_add_article): ?>
                <div class="add-btn-container">
                    <a href="add_pvo.php?type=<?php echo $pvo_type; ?>" class="add-btn">+ Добавить систему ПВО</a>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="filter-section" id="filterSection">
            <form method="GET" class="filter-form" id="filterForm">
                <input type="hidden" name="type" value="<?php echo $pvo_type; ?>">
        ...
        <div class="results-count">
            Найдено систем ПВО: <?php echo $pvo_count; ?>
        </div>

        <?php if ($pvo_count === 0): ?>
            <div class="no-vehicles">
                <p><?php echo $pvo_type_names[$pvo_type]; ?> не найдены</p>
                <?php if ($filter_country > 0 || $filter_service || $filter_year_from > 0 || $filter_year_to > 0): ?>
        ...
        <?php else: ?>
            <div class="vehicle-list">
                <?php foreach ($pvo_systems as $pvo): ?>
                    <div class="vehicle-item">
                        <?php if (!empty($pvo['ImagePath'])): ?>
                            <img src="../<?php echo htmlspecialchars($pvo['ImagePath']); ?>" 
                                 alt="<?php echo htmlspecialchars($pvo['Name']); ?>" 
                                 class="vehicle-image">
                        <?php else: ?>
                        ...
                        <div class="vehicle-content">
                            <a href="pvo_detail.php?id=<?php echo $pvo['ID']; ?>&from=pvo&type=<?php echo $pvo_type; ?>" class="vehicle-name-link">
                                <h3 class="vehicle-name"><?php echo htmlspecialchars($pvo['Name']); ?></h3>
                            </a>
                            ...
                            <div class="vehicle-details">
                                <?php if (!empty($pvo['CountryName'])): ?>
                                    <div class="detail-group">
                                        ...
                                        <span class="detail-value"><?php echo htmlspecialchars($pvo['CountryName']); ?></span>
                                    </div>
                                <?php endif; ?>
                                
                                <?php if (!empty($pvo['TypeName'])): ?>
                                    <div class="detail-group">
                                        <span class="detail-label">Тип:</span>
                                        <span class="detail-value"><?php echo htmlspecialchars($pvo['TypeName']); ?></span>
                                    </div>
                                <?php endif; ?>
                                ...
                                <?php if (!empty($pvo['WeaponName'])): ?>
                                    <div class="detail-group">
                                        ...
                                        <span class="detail-value"><?php echo htmlspecialchars($pvo['WeaponName']); ?></span>
                                    </div>
                                <?php endif; ?>
                                
                                <?php if (!empty($pvo['WeaponCalibre'])): ?>
                                    <div class="detail-group">
                                        <span class="detail-label">Калибр/Дальность:</span>
                                        <span class="detail-value"><?php echo htmlspecialchars($pvo['WeaponCalibre']); ?></span>
                                    </div>
                                <?php endif; ?>
                                ...
                                <?php if (!empty($pvo['Year_of_commissioning'])): ?>
                                    <div class="detail-group">
                                        ...
                                        <span class="detail-value"><?php echo htmlspecialchars($pvo['Year_of_commissioning']); ?></span>
                                    </div>
                                <?php endif; ?>
                            ...
                            <?php if ($can_delete_article): ?>
                                <div class="vehicle-actions">
                                    <button type="button" class="delete-btn" 
                                            onclick="showDeleteModal(<?php echo $pvo['ID']; ?>, '<?php echo htmlspecialchars(addslashes($pvo['Name'])); ?>')">
                                        Удалить
                                    </button>
                                </div>
                            <?php endif; ?>
    ...
    <div class="modal-overlay" id="deleteModal">
        <div class="modal-content">
            ...
            <p class="modal-text" id="deleteModalText">
                Вы действительно хотите удалить систему ПВО?
            </p>
            <div class="modal-actions">
                <form method="POST" id="deleteForm">
                    <input type="hidden" name="pvo_id" id="deletePvoId">
                    <input type="hidden" name="delete_pvo" value="1">
    ...
    <script>
        ...
        function resetFilters() {
            ...
            url.search = '?type=' + <?php echo $pvo_type; ?>;
        ...
        function showDeleteModal(pvoId, pvoName) {
            const modal = document.getElementById('deleteModal');
            const modalText = document.getElementById('deleteModalText');
            const deleteForm = document.getElementById('deleteForm');
            const deletePvoId = document.getElementById('deletePvoId');
            
            modalText.textContent = `Вы действительно хотите удалить систему ПВО "${pvoName}"? Это действие нельзя отменить.`;
            deletePvoId.value = pvoId;
