...
// Получаем тип средств разведки из параметра URL
...
$page_title = "Добавить " . mb_strtolower($recon_type_names[$recon_type]);
...
if (!$can_add_article) {
    header("Location: recon.php?type=" . $recon_type);
    exit();
}
...
// Классы (ищем средства разведки - Class_ID = 3)
$class_stmt = $mysqli->prepare("SELECT ID, Name FROM Class WHERE ID = 3 ORDER BY Name");
if ($class_stmt) {
    $class_stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $class_id = 3; // Средства разведки
    $service_start = trim($_POST['service_start'] ?? '');
    $service_end = trim($_POST['service_end'] ?? '');
    $generation_id = $recon_type; // Используем переданный тип разведки как Generation_ID
    $type_id = 2; // Устанавливаем Type_ID = 2 (наземная техника)

    // Валидация
    if (empty($name)) {
        $error_message = 'Название средства разведки обязательно для заполнения';
        ...
        // Вставляем данные в базу с указанием типа разведки как Generation_ID и Type_ID = 2
        ...
        $insert_stmt->bind_param("ssiiiiiii", 
            ...
            $class_id, 
            $service_id, 
            $generation_id, // Используем recon_type как Generation_ID
            $type_id  // Добавляем Type_ID = 2 (наземная техника)
        );
        
        if ($insert_stmt->execute()) {
            $vehicle_id = $mysqli->insert_id;
            
            // Логируем создание средства разведки
            $logger->logCreate('Vehicle', $vehicle_id, "Добавлено средство разведки: " . $name);
            
            // Обработка загрузки изображения
            if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                $upload_dir = '../uploads/'; // Исправлен путь - добавлен ../
                if (!is_dir($upload_dir)) {
                    mkdir($upload_dir, 0755, true);
                }
                
                $file_extension = pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION);
                $filename = 'recon_' . $vehicle_id . '_' . time() . '.' . $file_extension;
                ...
                if (move_uploaded_file($_FILES['image']['tmp_name'], $filepath)) {
                    ...
                    $image_path_in_db = 'uploads/' . $filename; // Путь для базы данных без ../
                    $image_stmt = $mysqli->prepare("INSERT INTO vehicle_images (Vehicle_ID, ImagePath, ImageType) VALUES (?, ?, ?)");
                    $image_stmt->bind_param("iss", $vehicle_id, $image_path_in_db, $image_type);
                    $image_stmt->execute();
                    $image_stmt->close();
                    
                    // Логируем добавление изображения
                    $logger->logCreate('vehicle_images', $mysqli->insert_id, "Добавлено изображение для средства разведки: " . $name);
                }
            }
            
            // Устанавливаем сообщение об успехе
            $_SESSION['success_message'] = 'Средство разведки успешно добавлено!';
            
            // Перенаправляем на страницу со списком средств разведки соответствующего типа
            header("Location: recon.php?type=" . $recon_type . "&added=1");
            exit();
            
        } else {
            $error_message = 'Ошибка при добавлении средства разведки: ' . $mysqli->error;
        }
        $insert_stmt->close();
    }
}
...
<body>
    ...
    <div class="main-content">
        <div class="page-header">
            <a href="recon.php?type=<?php echo $recon_type; ?>" class="back-btn">← Назад к списку средств разведки</a>
        </div>
        ...
        <form class="add-form" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">
            <!-- Скрытое поле для передачи типа разведки -->
            <input type="hidden" name="type" value="<?php echo $recon_type; ?>">
            
            <div class="form-group">
                <label class="form-label">Название средства разведки *</label>
            ...
            <div class="form-group">
                ...
                <div class="weapon-row">
                    ...
                    <div>
                        <label class="form-label">Калибр/Характеристики</label>
                        <input type="text" class="form-input" name="weapon_calibre" value="<?php echo htmlspecialchars($_POST['weapon_calibre'] ?? ''); ?>" placeholder="Калибр или характеристики">
                    </div>
                </div>
            ...
            <div class="form-group">
                ...
                <div class="service-row">
                    ...
                    <div>
                        ...
                        <small style="color: #8b966c; font-size: 0.8rem; display: block; margin-top: 0.3rem;">Если оставить пустым, средство разведки будет отмечено как "в эксплуатации"</small>
                    </div>
                </div>
            </div>
            ...
            <button type="submit" class="submit-btn">Добавить средство разведки</button>
        </form>
    </div>
