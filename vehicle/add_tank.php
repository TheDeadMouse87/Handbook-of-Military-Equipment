...
// Получаем тип танка из параметра URL
...
$page_title = "Добавить " . mb_strtolower($tank_type_names[$tank_type]);
...
if (!$can_add_article) {
    header("Location: tanks.php?type=" . $tank_type);
    exit();
}
...
// Классы (ищем танки - Class_ID = 1)
$class_stmt = $mysqli->prepare("SELECT ID, Name FROM Class WHERE ID = 1 ORDER BY Name");
if ($class_stmt) {
    $class_stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $generation_id = $tank_type; // Используем переданный тип танка как Generation_ID
    $type_id = 2; // Устанавливаем Type_ID = 2 (наземная техника)

    // Валидация
    if (empty($name)) {
        $error_message = 'Название танка обязательно для заполнения';
        ...
        // Вставляем данные в базу с указанием типа танка как Generation_ID и Type_ID = 2
        ...
        $insert_stmt->bind_param("ssiiiiiii", 
            ...
            $class_id, 
            $service_id, 
            $generation_id, // Используем tank_type как Generation_ID
        ...
        if ($insert_stmt->execute()) {
            ...
            // Логируем создание танка
            $logger->logCreate('Vehicle', $vehicle_id, "Добавлен новый танк: " . $name);
            ...
            if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                ...
                $filename = 'tank_' . $vehicle_id . '_' . time() . '.' . $file_extension;
                ...
                if (move_uploaded_file($_FILES['image']['tmp_name'], $filepath)) {
                    ...
                    $logger->logCreate('vehicle_images', $mysqli->insert_id, "Добавлено изображение для танка: " . $name);
                }
            }
            
            // Устанавливаем сообщение об успехе
            $_SESSION['success_message'] = 'Танк успешно добавлен!';
            
            // Перенаправляем на страницу со списком танков соответствующего типа
            header("Location: tanks.php?type=" . $tank_type . "&added=1");
            exit();
            
        } else {
            $error_message = 'Ошибка при добавлении танка: ' . $mysqli->error;
        }
        $insert_stmt->close();
    }
}
...
<body>
    ...
    <div class="main-content">
        <div class="page-header">
            <a href="tanks.php?type=<?php echo $tank_type; ?>" class="back-btn">← Назад к списку танков</a>
        </div>
        ...
        <form class="add-form" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">
            <!-- Скрытое поле для передачи типа танка -->
            <input type="hidden" name="type" value="<?php echo $tank_type; ?>">
            
            <div class="form-group">
                <label class="form-label">Название танка *</label>
            ...
            <div class="form-group">
                ...
                <div class="service-row">
                    ...
                    <div>
                        ...
                        <small style="color: #8b966c; font-size: 0.8rem; display: block; margin-top: 0.3rem;">Если оставить пустым, танк будет отмечен как "в эксплуатации"</small>
                    </div>
                </div>
            </div>
